...
public class ToolBean {
    ...
    @Id // フィールドを主キーとしてマーク
    @Column(name = "id", nullable = false) // カラム名を指定し、NOT NULL制約
    ...
    @Column(name = "productName", nullable = false) // カラム名を指定し、NOT NULL制約
    private String productName; //品名
    
    @Column(name = "modelNumber", nullable = false) // カラム名を指定し、NOT NULL制約
    private String modelNumber; //型番
    
    @Column(name = "maker", nullable = false) // カラム名を指定し、NOT NULL制約
    private String maker; //メーカー
    
    @Column(name = "purchaseDate", nullable = false) // カラム名を指定し、NOT NULL制約
    private Date purchaseDate; //購入日 yyyy-mm-dd
    
    @Column(name = "serviceLife", nullable = false) // カラム名を指定し、NOT NULL制約
    private int serviceLife; //耐用年数
    
    @Column(name = "depreciation", nullable = false) // カラム名を指定し、NOT NULL制約
    private int depreciation; //減価償却
    
    @Column(name = "usageProhibited", nullable = false) // カラム名を指定し、NOT NULL制約
    private boolean usageProhibited; //使用不可
    
    @Column(name = "availableForRent", nullable = false) // カラム名を指定し、NOT NULL制約
    private boolean availableForRent; //貸出可能
    
    @Column(name = "installationLocation", nullable = false) // カラム名を指定し、NOT NULL制約
